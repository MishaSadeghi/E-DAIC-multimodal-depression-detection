...
class PHQDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #         features_df = features_df.drop(index=indices_to_remove)
        #         features_df = features_df.dropna()
        #         features_df = features_df.drop(['confidence', 'success'], axis=1)
        #         filtered_features.extend(features_df[pose_columns].values)
        #     elif self.feature_type == 'gaze':
        #         gaze_columns = [col for col in features_df.columns if col.startswith('gaze_')]
        #         indices_to_remove = features_df[features_df['confidence'] < 0.9].index
        #         features_df = features_df.drop(index=indices_to_remove)
        #         features_df = features_df.dropna()
        #         features_df = features_df.drop(['confidence', 'success'], axis=1)
        #         filtered_features.extend(features_df[gaze_columns].values)
        #     elif self.feature_type == 'au_r':
        #         au_r_columns = [col for col in features_df.columns if col.endswith('_r')]
        #         indices_to_remove = features_df[features_df['confidence'] < 0.9].index
        #         features_df = features_df.drop(index=indices_to_remove)
        #         features_df = features_df.dropna()
        #         features_df = features_df.drop(['confidence', 'success'], axis=1)
        #         filtered_features.extend(features_df[au_r_columns].values)
        #     elif self.feature_type == 'au_c':
        #         au_c_columns = [col for col in features_df.columns if col.endswith('_c')]
        #         indices_to_remove = features_df[features_df['confidence'] < 0.9].index
        #         features_df = features_df.drop(index=indices_to_remove)
        #         features_df = features_df.dropna()
        #         features_df = features_df.drop(['confidence', 'success'], axis=1)
        #         filtered_features.extend(features_df[au_c_columns].values)
        #     elif self.feature_type == 'confidence_success_pose':
        #         pose_columns = [col for col in features_df.columns if col.startswith('pose_')]
        #         confidence_success_columns = ['confidence', 'success']
        #         selected_columns = pose_columns + confidence_success_columns
        #         features_df = features_df.dropna()
        #         filtered_features.extend(features_df[selected_columns].values)
        #     elif self.feature_type == 'confidence_success_gaze':
        #         gaze_columns = [col for col in features_df.columns if col.startswith('gaze_')]
        #         confidence_success_columns = ['confidence', 'success']
        #         selected_columns = gaze_columns + confidence_success_columns
        #         features_df = features_df.dropna()
        #         filtered_features.extend(features_df[selected_columns].values)
        #     elif self.feature_type == 'confidence_success_AUintens':
        #         au_r_columns = [col for col in features_df.columns if col.endswith('_r')]
        #         confidence_success_columns = ['confidence', 'success']
        #         selected_columns = au_r_columns + confidence_success_columns
        #         features_df = features_df.dropna()
        #         filtered_features.extend(features_df[selected_columns].values)
        #     elif self.feature_type == 'confidence_success_AUoccurr':
        #         au_c_columns = [col for col in features_df.columns if col.endswith('_c')]
        #         confidence_success_columns = ['confidence', 'success']
        #         selected_columns = au_c_columns + confidence_success_columns
        #         features_df = features_df.dropna()
        #         filtered_features.extend(features_df[selected_columns].values)
        #     elif self.feature_type == 'confidence_success_AUoccurr_pose_gaze':
        #         au_c_columns = [col for col in features_df.columns if col.endswith('_c')]
        ...
        #         pose_columns = [col for col in features_df.columns if col.startswith('pose_')]
        #         confidence_success_columns = ['confidence', 'success']
        #         selected_columns = au_c_columns + gaze_columns + pose_columns + confidence_success_columns
        #         features_df = features_df.dropna()
        #         filtered_features.extend(features_df[selected_columns].values)
        #     elif self.feature_type == 'confidence_success_AUintens_pose_gaze':
        #         au_r_columns = [col for col in features_df.columns if col.endswith('_r')]
        ...
        #         pose_columns = [col for col in features_df.columns if col.startswith('pose_')]
        #         confidence_success_columns = ['confidence', 'success']
        #         selected_columns = au_r_columns + gaze_columns + pose_columns + confidence_success_columns
        #         features_df = features_df.dropna()
        #         filtered_features.extend(features_df[selected_columns].values)
        ...
        # else:
        #     features = features_df.values
        #     # print('2 features: ', features.shape)
...
for feature_type in feature_combinations:
    ...
    optimizer = optim.Adam(model.parameters(), lr=0.01)  # lr=0.1 wasn't good. 
    models.append(model)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for i, (features, phq_scores, _) in enumerate(dev_loader):
                ...
                print("LSTM - Dev set - True and Predicted Values:")
                for true, pred in zip(phq_scores.cpu().numpy(), outputs.cpu().numpy()):
    ...
    with torch.no_grad():
        for i, (features, phq_scores, _) in enumerate(test_loader):
            ...
            print("LSTM - Test set - True and Predicted Values:")
            for true, pred in zip(phq_scores.cpu().numpy(), outputs.cpu().numpy()):
                print("True:", true, "\tPredicted:", pred)
